# [微服务权限终极解决方案，Spring Cloud Gateway + Oauth2 实现统一认证和鉴权！](http://www.macrozheng.com/#/cloud/gateway_oauth2?id=微服务权限终极解决方案，spring-cloud-gateway-oauth2-实现统一认证和鉴权！)
...
## [前置知识](http://www.macrozheng.com/#/cloud/gateway_oauth2?id=前置知识)
...
## [应用架构](http://www.macrozheng.com/#/cloud/gateway_oauth2?id=应用架构)
...
## [方案实现](http://www.macrozheng.com/#/cloud/gateway_oauth2?id=方案实现)

> 下面介绍下这套解决方案的具体实现，依次搭建认证服务、网关服务和API服务。

### [micro-oauth2-auth](http://www.macrozheng.com/#/cloud/gateway_oauth2?id=micro-oauth2-auth)
...
```bash
keytool -genkey -alias jwt -keyalg RSA -keystore jwt.jksCopy to clipboardErrorCopied
```
...
### [micro-oauth2-gateway](http://www.macrozheng.com/#/cloud/gateway_oauth2?id=micro-oauth2-gateway)
...
secure:
  ignore:
    urls: #配置白名单路径
      - "/actuator/**"
      - "/auth/oauth/token"Copy to clipboardErrorCopied
```
...
### [micro-oauth2-api](http://www.macrozheng.com/#/cloud/gateway_oauth2?id=micro-oauth2-api)
...
## [功能演示](http://www.macrozheng.com/#/cloud/gateway_oauth2?id=功能演示)
...
![img](http://www.macrozheng.com/images/gateway_oauth2_01.png)

- 使用密码模式获取JWT令牌，访问地址：http://localhost:9201/auth/oauth/token

![img](http://www.macrozheng.com/images/gateway_oauth2_02.png)

- 使用获取到的JWT令牌访问需要权限的接口，访问地址：http://localhost:9201/api/hello

![img](http://www.macrozheng.com/images/gateway_oauth2_03.png)

- 使用获取到的JWT令牌访问获取当前登录用户信息的接口，访问地址：http://localhost:9201/api/user/currentUser

![img](http://www.macrozheng.com/images/gateway_oauth2_04.png)

- 当JWT令牌过期时，使用refresh_token获取新的JWT令牌，访问地址：http://localhost:9201/auth/oauth/token

![img](http://www.macrozheng.com/images/gateway_oauth2_05.png)

- 使用没有访问权限的`andy`账号登录，访问接口时会返回如下信息，访问地址：http://localhost:9201/api/hello

![img](http://www.macrozheng.com/images/gateway_oauth2_06.png)