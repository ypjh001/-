...
### 2.2 打开只读文件,修改后需要保存时(不用切换用户即可保存的方式)
...
### 5.6 vmstat, 给定时间监控CPU使用率, 内存使用, 虚拟内存交互, IO读写
